...
[
V: \mathbb{R}^d \rightarrow [1,\infty),
]

we study the drift quantity
...
where:
- (Y\sim Q(x,\cdot)) is the MALA proposal;
- (\alpha(x,y)) is the Metropolis-Hastings acceptance probability.

The drift integral is decomposed into three regions:

- (A_1 := {y\in\mathbb{R}^d:|y|<|x|-\varepsilon}) represents inward moves;
- (A_2 := {y\in\mathbb{R}^d:|x|-\varepsilon \le |y| \le |x|+\varepsilon}) represents local moves;
- (A_3 := {y\in\mathbb{R}^d:|y|>|x|+\varepsilon}) represents outward moves.
